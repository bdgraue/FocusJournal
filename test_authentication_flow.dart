...
void main() {
  group('Authentication Flow Tests', () {
    ...
    testWidgets('First app start should show method selection screen', (
      WidgetTester tester,
    ) async {
    ...
    testWidgets('User can select any authentication method on first setup', (
      WidgetTester tester,
    ) async {
      SharedPreferences.setMockInitialValues({});
    ...
    test(
      'Authentication service allows switching methods without keeping password backup',
      () async {
        ...
        expect(
          await authService.getCurrentAuthMethod(),
          equals(AuthenticationService.authMethodPassword),
        );
        ...
        expect(
          await authService.getCurrentAuthMethod(),
          equals(AuthenticationService.authMethodPin),
        );

        // Current implementation doesn't keep password as backup
        // This test demonstrates the issue
        final canAuthWithPassword = await authService.verifyPassword(
          'testpassword123',
        );
        print(
          'Can still authenticate with original password after PIN setup: $canAuthWithPassword',
        );
      },
    );
  });
